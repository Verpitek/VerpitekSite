...
const ELECTRIC_FURNACE_SOUND_ID = "lunatech:electric_furnace_sound";
...
MachineCustomComponent.registerTickEvent((ev, params) => {
    ...
    let soundEnt = linkedEnts.find((ent) => {
        return ent.typeId === ELECTRIC_FURNACE_SOUND_ID
    });

    if (soundEnt === undefined) {
        // Needs created
        soundEnt = ev.dimension.spawnEntity(ELECTRIC_FURNACE_SOUND_ID, ev.block.bottomCenter(), {});
    ...
    machineId: "lunatech:electric_furnace",
})
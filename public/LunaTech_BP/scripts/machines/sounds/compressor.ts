...
const COMPRESSOR_SOUND_ID = "lunatech:compressor_sound";
...
MachineCustomComponent.registerTickEvent((ev, params) => {
    ...
    let soundEnt = linkedEnts.find((ent) => {
        return ent.typeId === COMPRESSOR_SOUND_ID
    });

    if (soundEnt === undefined) {
        // Needs created
        soundEnt = ev.dimension.spawnEntity(COMPRESSOR_SOUND_ID, ev.block.bottomCenter(), {});
